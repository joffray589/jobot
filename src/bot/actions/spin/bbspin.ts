/**
 *  https://github.com/BaliBalo/bbbbot/blob/master/scripts/spin/index.js
 */
...
const shuffle = (a) => a.map((v,i,j)=>a[a[i]=a[j=0|i+Math.random()*(a.length-i)],j]=v);
...
const replaceStandardEmojis = (txt: string) => {
    const imgtags = /<img class="emoji"[^>]* src="([^"]+)"[^>]*\/>/g;
    return twemoji.parse(txt, icon => "[[icon=https://twemoji.maxcdn.com/2/72x72/"+icon+".png]]").replace(imgtags, (m, src) => src);
}

const fillText = (ctx, text, x, y, max, asImage?:any) => {
    let parts = [];
    let token;
...
export let spin = async (choices, interaction) =>  {
    ...
    let file = new MessageAttachment(stream, 'spin.gif');
